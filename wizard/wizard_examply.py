...
class MagicWizard(QtWidgets.QWizard):
    ...
    def onFinish(self):  # this could be where we do all of the hdf5 creation etc. That would be best as the file won't be created if the user presses the cancel button.
        ...
        # Create HDF5 project file
        if self.spectrum_hdr and self.spectrum_dat:
            project_filename = f"{self.project_title.replace(' ', '_')}.h5" if self.project_title else "new_project.h5"
            
            try:
                fileh = tb.open_file(project_filename, 'w')
                
                # Create main groups
                fileh.create_group(fileh.root, 'levels', 'Levels')
                fileh.create_group(fileh.root, 'spectra', 'Spectra')
                fileh.create_group(fileh.root, 'previousLines', 'Previous Lines')
                fileh.create_group(fileh.root, 'calculations', 'Calculations')
                fileh.create_group(fileh.root, 'matched_lines', 'Matched Lines')
                
                # Import spectrum with optional linelist
                linelist_file = self.linelist_file if self.linelist_file else None
                import_spectrum_with_linelist(fileh, self.spectrum_dat, self.spectrum_hdr, linelist_file)
                
                fileh.close()
                print(f"Project created successfully: {project_filename}")
                
            except Exception as e:
                print(f"Error creating project: {str(e)}")
        else:
            print("Missing required spectrum files (.dat/.hdr)")
...
class Page1(QtWidgets.QWizardPage):
    def __init__(self, parent=None):
        ...
        self.SpectrumFile = QPushButton("Add Spectrum and Linelist")
...
#            print(spectrum[0])
            if fname[0][-4:] == ".hdr":
                ...
                # Now ask for optional linelist file
                linelist_fname = QFileDialog.getOpenFileName(self, "Select Linelist File (Optional)", "", 
                                                           "Linelist Files (*.aln *.lst *.wavcorr);;All Files (*)")
                if linelist_fname and linelist_fname[0]:
                    self.wizard().linelist_file = linelist_fname[0]
                    print(f"Selected linelist: {linelist_fname[0]}")
                else:
                    self.wizard().linelist_file = ''
                    print("No linelist selected - spectrum only")
                
                self.SpectrumFile.setIcon(self.tick_icon)  # add a tick to show the user it has been completed.
                
    
    def _add_linelist(self):             
        fname = QFileDialog.getOpenFileName(self, "Select Linelist File", "", "Linelist Files (*.txt);;All Files (*)")
        if fname:                
            self.wizard().linelist_file = fname[0]   
            self.LinesFile.setIcon(self.tick_icon) 
            
    def _add_levels(self):             
        fname = QFileDialog.getOpenFileName(self, "Select Levels File", "", "Level Files (*.lev);;All Files (*)")
        if fname:                
            self.wizard().level_file = fname[0]   
            self.LevelsFile.setIcon(self.tick_icon) 
            
    def _add_calcs(self):             
        fname = QFileDialog.getOpenFileName(self, "Select Calculations File", "", "Calculation Files (*.txt);;All Files (*)")
        if fname:                
            self.wizard().calc_file = fname[0]  
            self.CalcsFile.setIcon(self.tick_icon) 
            
    def _add_prev_idents(self):             
        fname = QFileDialog.getOpenFileName(self, "Select Previous Identifications File", "", "Identification Files (*.txt);;All Files (*)")
        if fname:                
...
class Page2(QtWidgets.QWizardPage):
    ...
    def initializePage(self):
        self.label1.setText("Spectrum:" + self.wizard().spectrum_hdr)
        self.label2.setText("Linelist: " + self.wizard().linelist_file)
