...
class linelistTableModel(QtCore.QAbstractTableModel):
    """Model for the linelist table view in the main window. Takes a Pandas dataframe and displays the linelist data automatically."""
    def __init__(self, data):
        super(linelistTableModel, self).__init__()
        self._data = data

    def data(self, index, role):
        """The data to be displayed. This is the linelist dataframe"""
        if role == Qt.DisplayRole:
            value = self._data.iloc[index.row(), index.column()]
            
            if isinstance(value, bytes):  # changes from byte string to utf-8 for display only
                value = value.decode('utf-8')
    ...
    def headerData(self, section, orientation, role):
        """Custom header labels for the linelist table columns"""
        if role == Qt.DisplayRole:
            if orientation == Qt.Horizontal:
