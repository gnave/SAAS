# importers.py (Complete, Final Version)
...
def parse_hdr_file(header_filepath: str):
    """
    Reads a spectrometer .hdr file and returns a metadata dictionary.
    """
    metadata = {}
    last_key = None
...
def import_spectrum_pair(h5_filepath, raw_data_file, header_file, is_calibration_spectrum=False, target_spectrum_group=None):
    """
    Imports a spectrum pair. If it's a main spectrum, it creates the full nested
    structure. If it's a calibration spectrum, it places it within an existing
    spectrum's 'Calibration_Spectra' group.
    """
...
def import_binary_linelist(h5_filepath, lin_filepath, target_spectrum_group):
    """
    Reads a .lin file, applies conversions, and saves it to the HDF5 file.
    """
...
def import_calibrated_linelist(h5_filepath, txt_filepath, target_spectrum_group):
    """
    Reads a calibrated text linelist using a robust manual parser and saves to HDF5.
    """
    ...
    table_name = os.path.splitext(os.path.basename(txt_filepath))[0].replace('.','_')
...
def import_lamp_calibration(h5_filepath, cal_filepath, user_metadata):
    """
    Reads a standard lamp calibration file and saves it to the top-level
    /Standard_Lamp_Calibrations group.
    """
...
def parse_generic_text_file(filepath, file_type='delimited', delimiter=',', has_header=False, col_widths=None):
    """
    Parses a generic text file (CSV, whitespace-delimited, or fixed-width).
    """
